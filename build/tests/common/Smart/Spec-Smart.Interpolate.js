describe("Smart.Interpolate", function() {
	describe("Smart.Interpolate.numbers", function() {
		...
		it("should EXTRAPOLATE when given high and low pct", function() {
			expect([ -1, -0.25, 0.5, 1.25, 2 ].map(interpolate))
	...
	describe("Smart.Interpolate.colors", function() {
		...
		it("should interpolate between colors", function() {
			expect([ 0, 0.25, 0.5, 0.75, 1 ].map(interpolate))
				.toEqual([ 
					'rgb(100,110,120)'
					,'rgb(125,135,145)'
					,'rgb(150,160,170)'
		...
		it("should validate and cap values", function() {
			expect([ -2, -1, -0.25, -0.1, 1.1, 1.5, 2 ].map(interpolateCap))
				.toEqual([
					'rgb(0,0,0)'
					,'rgb(0,0,0)'
					,'rgb(75,75,75)'
		...
		it("should interpolate alphas", function() {
			expect([-1, -0.1, 0, 0.1, 0.5, 0.9, 1, 2].map(interpolateAlpha))
				.toEqual([
					'rgba(0,0,0,0.00)'
					,'rgba(0,0,0,0.09)'
					,'rgba(0,0,0,0.10)'
		...
		it("should interpolate and cap hsl values too", function() {
			expect([ -100, -2, -1, -0.1, 0, 0.1, 0.5, 0.9, 1, 1.1, 2, 100 ].map(interpolateHSL))
				.toEqual([
					'hsl(90,0%,0%)'
					,'hsl(350,0%,10%)'
					,'hsl(0,10%,20%)'
		...
		it("should interpolate hsla", function() {
			expect([ -100, -1, 0, 0.5, 1, 2, 100 ].map(interpolateHSLA))
				.toEqual([
					'hsla(0,0%,0%,0.00)'
					,'hsla(0,0%,0%,0.00)'
					,'hsla(0,0%,0%,0.10)'
