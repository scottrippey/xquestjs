export const Color = {
  parseRGB(color) {
    ...
    const hex6 = color.match(/^#?([A-Fa-f0-9]{2})([A-Fa-f0-9]{2})([A-Fa-f0-9]{2})$/);
    if (hex6) return hex6.slice(1).map((h) => parseInt(h, 16));

    const hex3 = color.match(/^#?([A-Fa-f0-9])([A-Fa-f0-9])([A-Fa-f0-9])$/);
    if (hex3) return hex3.slice(1).map((h) => parseInt(h, 16));

    const rgba = color.match(/^rgba?\((\d{1,3}),(\d{1,3}),(\d{1,3})(?:,(\d?\.?\d*))?\)$/);
    if (rgba) return rgba.slice(1, rgba[4] ? 5 : 4).map((d) => parseFloat(d));
  ...
  toRGB(array) {
    ...
    const r = str(array[0], 0, 255, 0);
    const g = str(array[1], 0, 255, 0);
    const b = str(array[2], 0, 255, 0);
    if (array.length === 4 && array[3] !== 1) {
      const a = str(array[3], 0, 1, 2);
      return `rgba(${r},${g},${b},${a})`;
    } else {
      return `rgb(${r},${g},${b})`;
    }
  ...
  parseHSL(color) {
    ...
    const hsla = color.match(/^hsla?\((\d{1,3}),(\d{1,3})%,(\d{1,3})%(?:,(\d?\.?\d*))?\)$/);
    if (hsla) return hsla.slice(1, hsla[4] ? 5 : 4).map((d) => parseFloat(d));
  ...
  toHSL(array) {
    ...
    hue = hue.toFixed(0);
    const sat = str(array[1], 0, 100, 0);
    const lum = str(array[2], 0, 100, 0);

    if (array.length === 4) {
      const alpha = str(array[3], 0, 1, 2);
